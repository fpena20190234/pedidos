...
class MemoDbProvider {
  ...
  Future<int> addItem(Logina item) async {
    //returns number of items inserted as an integer
    final db = await init(); //open database
    return db.insert(
      "Login", item.toMap(), //toMap() function from Logina
      conflictAlgorithm:
          ConflictAlgorithm.ignore, //ignores conflicts due to duplicate entries
    );
  }

  Future<List<Logina>> fetchLogin() async {
    //returns the memos as a list (array)

    final db = await init();
    final maps = await db
        .query("Login"); //query all the rows in a table as an array of maps
  ...
  Future<int> updateMemo(String usuario, Logina item) async {
    // returns the number of rows updated
    final db = await init();

    int result = await db.update("Login", item.toMap(),
  ...
  //   );
  // }
  Future<int> deleteLinieros() async {
    //returns number of items deleted
    final db = await init();
    int result = await db.delete(
      "Linieros", //table name
...
*/
  ...
  Future<int> updateSupervisores(String usuario, Logina item) async {
    // returns the number of rows updated
    final db = await init();
